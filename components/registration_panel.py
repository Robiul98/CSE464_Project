...
Three panels: F Grade | Retakable | General
"""
...
def _format_schedule(section_id: int) -> str:
    ...
    if not rows:
        return "—"
    parts = [f"{r['day_of_week']} {r['st']}-{r['et']} ({r['class_type']})" for r in rows]
    return " | ".join(parts)


def _enrolled_sections(student_id: str, semester_id: int) -> set:
    """Return set of section_ids the student is currently ENROLLED in this semester."""
    rows = fetch_all(
        """
        SELECT e.section_id
        FROM   enrollments e
...
def _f_grade_course_ids(student_id: str) -> set:
    rows = fetch_all(
        "SELECT course_id FROM takes WHERE user_id = :p_user_id AND grade = 'F'",
...
def _passed_course_ids(student_id: str) -> set:
    """Courses taken with a non-F, non-NULL grade (passed)."""
    rows = fetch_all(
        """
...
def _prereqs_met(student_id: str, course_id: int, passed_ids: set) -> bool:
    ...
    prereqs = fetch_all(
        "SELECT courses_course_id1 FROM course_prerequisites WHERE courses_course_id = :cid",
        {"cid": course_id},
    )
    if not prereqs:
        return True
...
def _sections_for_courses(course_ids: set, semester_id: int) -> list[dict]:
    """Return open sections for the given course_ids in the active semester."""
    if not course_ids:
        return []
...
def _do_enroll(student_id: str, section_id: int, actor_id: str,
    ...
    execute(
        ...
        SET    available_seats = available_seats - 1,
               section_status = CASE WHEN available_seats - 1 <= 0 THEN 'FULL' ELSE section_status END
...
def _do_drop(student_id: str, section_id: int, actor_id: str,
    ...
    execute(
        ...
        """,
        {"reason": drop_reason, "actor": actor_id, "sid": student_id, "sec": section_id},
        commit=False,
    )
...
def _render_section_row(row: dict, enrolled_sections: set, student_id: str,
    ...
    if is_enrolled:
        ...
        #     dl  = drop_deadline if isinstance(drop_deadline, datetime.datetime) else \
        #           datetime.datetime.combine(drop_deadline, datetime.time.max)
        #     can_drop = now <= dl
        if acting_role != "FACULTY":
            # 2. For Students (SELF), check the deadline
            if drop_deadline:
                now = datetime.datetime.now()
            ...
            if st.session_state.get(f"confirm_drop_{btn_key}"):
                with st.container(border=True):
                    ...
                    if c1.button("Confirm Drop", key=f"dropok_{btn_key}", type="primary"):
                        _do_drop(student_id, section_id, actor_id, acting_role,
                                 course_code, section_name, reason)
        ...
        if st.session_state.get(f"confirm_add_{btn_key}"):
            with st.container(border=True):
                ...
                if c1.button("Confirm", key=f"addok_{btn_key}", type="primary"):
                    _do_enroll(student_id, section_id, actor_id, acting_role,
                               course_code, section_name)
...
# ):
#     """
#     Render the three registration panels for a student.

#     Parameters
#     ----------
#     student_id  : the student being enrolled/dropped
#     acting_role : 'SELF' or 'FACULTY'
#     actor_id    : the user performing the action
#     semester_id : active semester pk
#     drop_deadline : datetime or date from semesters.drop_deadline
#     """
...
#     with st.expander("🔴 Panel A — F Grade Courses", expanded=True):
...
#             if not rows:
#                 st.info("Your F grade courses are not offered this semester.")
#             else:
#                 _panel_header()
#                 for row in rows:
#                     _render_section_row(row, enrolled, student_id, actor_id,
#                                         acting_role, drop_deadline, "panelA")
...
#         else:
#             # Exclude any already enrolled this semester from Panel B display
#             retake_rows = _sections_for_courses(p_ids, semester_id)
#             # Remove sections already enrolled
#             retake_rows = [r for r in retake_rows if r["section_id"] not in enrolled]
#             if not retake_rows:
#                 st.info("Your previously passed courses are not available for retake this semester.")
#             else:
#                 _panel_header()
#                 for row in retake_rows:
#                     _render_section_row(row, enrolled, student_id, actor_id,
#                                         acting_role, drop_deadline, "panelB")
...
#                 _render_section_row(row, enrolled, student_id, actor_id,
#                                     acting_role, drop_deadline, "panelC")

#     # ── Section Change (Faculty only) ─────────────────────────────────────────
#     if acting_role == "FACULTY":
...
def render_registration_panels(
    ...
    """
    Render the registration panels for a student.
    """
    enrolled = _enrolled_sections(student_id, semester_id)
    f_ids    = _f_grade_course_ids(student_id)
    p_ids    = _passed_course_ids(student_id)

    # ── NEW: Panel 0: Current Enrollments ─────────────────────────────────────
        ...
        if not enrolled:
            ...
            if enrolled_rows:
                ...
                for row in enrolled_rows:
                    _render_section_row(row, enrolled, student_id, actor_id,
                                        acting_role, drop_deadline, "panelE")
    ...
    with st.expander("🔴 Panel A — F Grade Courses", expanded=False):
        ...
        if not f_ids:
            ...
            rows = _sections_for_courses(f_ids, semester_id)
            # Hide if already enrolled (moved to Current Enrollments panel)
...
def _render_section_change(student_id: str, actor_id: str,
    ...
    """Faculty-only: move student from one section to another of the same course."""
    ...
    # Get enrolled courses this semester
    enrolled_rows = fetch_all(
        """
        SELECT e.section_id, c.course_id, c.course_code, c.course_title,
    ...
    selected_label = st.selectbox("Select current enrollment", list(course_options.keys()),
                                  key="sc_course_select")
    selected = course_options[selected_label]

    # Get other available sections for same course
    other_sections = fetch_all(
        """
        SELECT cs.section_id, cs.section_name, cs.available_seats, cs.section_status,
               f.faculty_name, cs.room_no
        ...
        """,
        {"cid": selected["course_id"], "sem": semester_id, "cur_sec": selected["section_id"]},
    )
    ...
    new_sec_label = st.selectbox("Move to section", list(sec_options.keys()), key="sc_new_sec")
    new_sec = sec_options[new_sec_label]

    if new_sec["section_status"] == "FULL":
