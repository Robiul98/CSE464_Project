...
def render():
    ...
    # 1. ADD 'tab_prefix' AS A PARAMETER
    def _render_messages(msg_list, tab_prefix):
        if not msg_list:
            st.info("No messages.")
            return
        for msg in msg_list:
            ...
            with st.expander(
                ...
                new_note = st.text_area(
                    "Admin Note (optional — visible to faculty on their sent messages)",
                    value=note_val,
                    # 2. ADD TAB PREFIX TO THE TEXT AREA KEY
                    key=f"note_{tab_prefix}_{mid}",
                )

                b1, b2, _ = st.columns([1.2, 1.2, 3])

                if is_unread:
                    # 3. ADD TAB PREFIX TO THE BUTTON KEY
                    if b1.button("✅ Mark as Read", key=f"read_{tab_prefix}_{mid}"):
                        execute(
                            """
                            UPDATE faculty_messages
                            SET    status = 'READ', read_at = SYSTIMESTAMP
                ...
                # 4. ADD TAB PREFIX TO THE BUTTON KEY
                if b2.button("💾 Save Note", key=f"savenote_{tab_prefix}_{mid}"):
                    execute(
                        "UPDATE faculty_messages SET admin_note = :note WHERE message_id = :mid",
                        {"note": new_note.strip() or None, "mid": mid},
                    )
    ...
    with tab1:
        # 5. PASS "unread" AS THE PREFIX
        _render_messages(unread, "unread")

    with tab2:
        # 6. PASS "all" AS THE PREFIX
        _render_messages(all_m, "all")
